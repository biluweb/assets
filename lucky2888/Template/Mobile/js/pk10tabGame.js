$(function() {
    ...
    function tabGameInit() {
        ...
        **/
        ...
        changeXYPGame();
        gameNumberXYP(pks_lmp_lmp1, pks_lmp_arr1, 24);
        gameNumberXYP(pks_lmp_lmp2, pks_lmp_arr2, 24);
    }
    ...
    $('.selectMultipInput').bind('click', function() {
        $(this).select();
    });
    ...
    function randomTouzhu() {
        ...
        if (_thisPlayid == 'lmp' || _thisPlayid == 'gyh' || _thisPlayid == 'yzwm') {
            //获取总个数
        ...
        switch (_thisType) {
            ...
            case 'gyh':
                $('.play_select_prompt').find('span[way-data="tabDoc"]')
                    .html('信用盘为经典玩法，下注金额为<em style="color:red;">1元</em>投注模式。'); //+ rates.maxjj +
                changeXYPGame();
                gameNumberXYP(pks_gyh, pks_gys_arr, 24);
                break;
            case 'yzwm':
                $('.play_select_prompt').find('span[way-data="tabDoc"]')
                    .html('信用盘为经典玩法，下注金额为<em style="color:red;">1元</em>投注模式。'); //+ rates.maxjj +
                changeXYPGame();
                gameNumberXYP(pks_yzwm, pks_yzsm_arr, 24);
                break;
            case 'lzsm':
                $('.play_select_prompt').find('span[way-data="tabDoc"]')
                    .html('信用盘为经典玩法，下注金额为<em style="color:red;">1元</em>投注模式。'); //+ rates.maxjj +
                changeXYPGame();
                gameNumberXYP(pks_lzsm, pks_yzsm_arr, 24);
                break;
                break;
        }
    ...
    function gameNumberXYP(arr, title_arr, width) {
        ...
        for (var i = 0; i < Object.keys(arr).length; i++) {
            ...
            for (var k = 0; k < title_arr.length; k++) {
                ...
                var rate = yrates[playid] ? yrates[playid].rate : "";
            ...
            game_area.append(game_balls);
            box.append(game_area);
        }
    }
    ...
    $('.betting_right_btn').on('click', function() {
        ...
        $('.g_Number_Section li>.active').each(function() {
            ...
            var html = '<dd class="yBettingList" id="' + arr.trano + '">' +
                '<div class="numberBox yBettingDiv">' +
                '<span class="number"> <em>' + rate.title + '</em></span>' +
                '</div>' +
                '<div class="yBettingType">[' + menu0 + ',' + menu1 + ',' + menu2 + ']</div>' +
                '<div class="yBettingZhushu yBettingDiv">' +
                '<em>' + arr.zhushu + '</em>注×' +
                '</div>' +
                '<div class="yBettingTimes yBettingDiv">' +
                '<em class="yBettingTimess">' + arr.beishu + '</em>倍×' +
                '</div>' +
                '<div class="rmb yBettingDiv">' +
                '' + arr.price + '元' +
                '</div>' +
                '<div class="yzongRmb" style="float: left;padding-left: 5px;">' +
                ' = ' + (parseInt(arr.zhushu) * parseInt(arr.beishu) * parseInt(price)) + '元' +
                '</div>' +
                '<div class="sc" style="float: right;padding-right: 5px;">' +
                '<a href="javascript:void(0);">' +
                '<i class="fa fa-times" style="color: red;"></i>' +
                '</a>' +
                '</div>' +
                '<div id="betting_money" style="display: none;">' + price + '</div>' +
                '</dd>';
            $('.yBettingLists').append(html);
        });
